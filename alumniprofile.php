...
session_start();
  ...
  {
    $q='select fname,lname,username,dob,email,gender,course,yr_passing,dept,details,status,cgpa,email,file from `alumni` where `username`="'.$_SESSION['uname'].'"';
...
<form role="form" method="post"><div id="data" class="data"><b>Deptartment:  </b> ' . $row['dept'].'<button type="button" class="nobutton" onclick="showdept()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
          ...
          echo '<div id="yr_passing" class="updatebox" style="display:none"><form method="post" role="form" >Enter updated Year of Passing<input type="number" name="yr_passingnew" value=""><button class="subbutton" type="submit" name="submityr_passing">Update</button></form></div>';
          echo '<form role="form" method="post"><div id="data" class="data"><b>CGPA: </b>' . $row['cgpa'].'<button type="button" class="nobutton" onclick="showcgpa()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
          echo '<div id="cgpa" class="updatebox" style="display:none"><form method="post" role="form" >Enter updated CGPA<input type="number" name="cgpanew" value=""><button class="subbutton" type="submit" name="submitcgpa">Update</button></form></div>';

          echo '</div><div class="parts" name="current"><font size="6" face="Hectiva" color="navy" style=" text-shadow: 1px 1px;"><b>Other Details</b></font><form role="form" method="post"><div id="data" class="data">
<b>Status:</b> ' . $row['status'].'<button type="button" class="nobutton" onclick="showstatus()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
          echo '<div id="status" class="updatebox" style="display:none"><form method="post" role="form" >Enter updated Status  <select name="statusnew">
              <option value="emp">Employed</option>
              <option value="self">Self Employed</option>
              <option value="other">other</option>
            </select><button class="subbutton" type="submit" name="submitstatus">Update</button></form></div>';
          echo '<form role="form" method="post"><div id="data" class="data"><b>Details: </b>' . $row['details'].'<button type="button" class="nobutton" onclick="showdetail()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
          echo '<div id="details" class="updatebox" style="display:none"><form method="post" role="form" >
              <textarea  rows="3" cols="50" value="" name="detailsnew">Enter other details</textarea><br>
              <button class="subbutton" type="submit" name="submitdetails">Update</button></form></div>
          </div>';
  }}
  if(isset($_POST['submitfile']))
  {
    ...
    $sql='UPDATE `alumni` SET `file`="'.$destinationfile.'" where `username`="'.$_SESSION['uname'].'"';
...
{
  $sql='UPDATE `alumni` SET `details`="'.$_POST['detailsnew'].'" where `username`="'.$_SESSION['uname'].'"';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
...
{
  $sql='UPDATE `alumni` set `fname`="'.$_POST['fnamenew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitlname']))
{
  $sql='UPDATE `alumni` set `lname`="'.$_POST['lnamenew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitdob']))
{
  $sql='UPDATE `alumni` set `dob`="'.$_POST['dobnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitgender']))
{
  $sql='UPDATE `alumni` set `gender`="'.$_POST['gendernew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitdept']))
{
  $sql='UPDATE `alumni` set `dept`="'.$_POST['deptnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitstatus']))
{
  $sql='UPDATE `alumni` set `status`="'.$_POST['statusnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
if(isset($_POST['submityr_passing']))
{
  $sql='UPDATE `alumni` set `yr_passing`="'.$_POST['yr_passingnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
if(isset($_POST['submitcgpa']))
{
  $sql='UPDATE `alumni` set `cgpa`="'.$_POST['cgpanew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
if(isset($_POST['submitemail']))
{
  $sql='UPDATE `alumni` set `email`="'.$_POST['emailnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
if(isset($_POST['submitcourse']))
{
  $sql='UPDATE `alumni` set `course`="'.$_POST['coursenew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
ob_end_flush();
