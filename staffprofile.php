...
session_start();
  ...
  {
    $q='select fname,lname,username,dob,email,gender,yr_joining,dept,email,file from `staff` where `username`="'.$_SESSION['uname'].'"';
...
<form role="form" method="post"><div id="data" class="data"><b>First Name:</b>  ' . $row['fname'].'<button type="button" class="nobutton" onclick="showfname()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
          ...
          echo '</div><div class="parts" name="institutional"><font size="6" face="Hectiva" color="navy" style=" text-shadow: 1px 1px;"><b>institutional Details</b></font>
<form role="form" method="post"><div id="data" class="data"><b>Deptartment:  </b> ' . $row['dept'].'<button type="button" class="nobutton" onclick="showdept()"><img src="edit.png" height="20px" width="20px"/></button></div></form>';
  ...
  if(isset($_POST['submitfile']))
  {
    ...
    $sql='UPDATE `staff` SET `file`="'.$destinationfile.'" where `username`="'.$_SESSION['uname'].'"';
...
{
  $sql='UPDATE `staff` SET `details`="'.$_POST['detailsnew'].'" where `username`="'.$_SESSION['uname'].'"';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
...
{
  $sql='UPDATE `staff` set `fname`="'.$_POST['fnamenew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitlname']))
{
  $sql='UPDATE `staff` set `lname`="'.$_POST['lnamenew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitdob']))
{
  $sql='UPDATE `staff` set `dob`="'.$_POST['dobnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitgender']))
{
  $sql='UPDATE `staff` set `gender`="'.$_POST['gendernew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
if(isset($_POST['submitdept']))
{
  $sql='UPDATE `staff` set `dept`="'.$_POST['deptnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
    header("Refresh:0");
}
...
{
  $sql='UPDATE `staff` set `yr_joining`="'.$_POST['yr_joiningnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
...
{
  $sql='UPDATE `staff` set `email`="'.$_POST['emailnew'].'" where `username`="'.$_SESSION["uname"].'";';
  $r=mysqli_query($conn,$sql);
  header("Refresh:0");
}
